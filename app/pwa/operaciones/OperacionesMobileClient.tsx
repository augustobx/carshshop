...
export default function OperacionesMobileClient({ operaciones }: { operaciones: any[] }) {
  ...
  const rows = useMemo(() => {
    ...
    return operaciones.filter((o) => {
      const isClosed = Boolean(o.ciclo_cerrado);
  ...
  return (
    <div className="min-h-screen bg-slate-50 pb-24">
      <header className="sticky top-0 z-40 rounded-b-3xl bg-slate-950 px-4 py-5 text-white shadow-lg">
        ...
        <div className="mt-1 flex items-end justify-between gap-3"><div><h1 className="text-2xl font-black">Mis operaciones</h1><p className="mt-1 text-xs text-slate-400">Desde la cotización hasta la entrega.</p></div><Link href="/pwa/cotizador" className="rounded-xl bg-blue-600 px-3 py-2 text-xs font-black">+ Cotizar</Link></div>
