...
function LoginForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!res.success) {
      ...
      if (res.user?.isSuperAdmin && !redirectParam) {
  ...
  return (
    <div className="bg-slate-900 border border-slate-800 rounded-3xl p-8 sm:p-10 w-full max-w-md shadow-2xl relative z-10 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="text-center mb-8">
        ...
        <h1 className="text-2xl sm:text-3xl font-black text-white tracking-tight">OnlyCars SaaS</h1>
        <p className="text-slate-400 font-medium text-sm mt-1.5">
          Plataforma Integral para Grandes Concesionarias
        </p>
      </div>
      ...
      {error && (
        <div className="bg-red-500/10 text-red-400 p-3.5 rounded-xl text-xs font-bold text-center mb-6 border border-red-500/20 animate-in shake">
          {error}
        </div>
      )}
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        <div className="space-y-1.5">
          ...
          <div className="relative">
            ...
            <input
              ...
              className="w-full pl-10 pr-4 py-2.5 bg-slate-950 border border-slate-800 rounded-xl font-medium text-sm text-white placeholder-slate-600 focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500 transition-all"
        ...
        <button
          type="submit"
          disabled={loading}
          className="w-full bg-blue-600 hover:bg-blue-500 text-white py-3 rounded-xl font-black text-sm flex justify-center items-center gap-2 shadow-lg shadow-blue-600/25 transition-all hover:scale-[1.01] active:scale-95 disabled:opacity-50 mt-2"
        >
      ...
      <div className="mt-8 pt-6 border-t border-slate-800/80 text-center">
        <p className="text-[11px] text-slate-500 uppercase tracking-widest font-bold">
          NanoLabs Multi-Tenant Automotive Cloud
        </p>
      </div>
    </div>
  );
