...
export default async function VehiculosPage({ searchParams }: { searchParams: Promise<any> }) {
    ...
    const where: any = { tenantId: tenant.id, tipo_vehiculo: 'Auto' };
    if (params.q) where.OR = [{ marca: { contains: params.q } }, { modelo: { contains: params.q } }, { patente: { contains: params.q } }];
    else {
    ...
    const vehiculosDb = await db.vehiculo.findMany({
        ...
        include: {
            ...
            senias: { where: { estado: 'ACTIVA' }, select: { id_senia: true, id_cliente: true, monto_ars: true, monto_usd: true, cliente: { select: { nombre_completo: true } } }, take: 1 },
        }
    });
    ...
    return <Suspense fallback={<div className="flex justify-center items-center h-screen"><Loader2 className="w-10 h-10 animate-spin text-blue-600" /></div>}><VehiculosClient vehiculos={vehiculos} currentTab={tab === 'senados' ? 'reservados' : tab} currentDolar={dolarBlue} /></Suspense>;
}
