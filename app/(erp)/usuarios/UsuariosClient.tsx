...
import { UserPlus, Trash2, Users, X, Loader2, Mail, Lock, Search, Percent } from 'lucide-react';
...
export default function UsuariosClient({ usuarios, locations, maxUsers, canCreateOwner }: { usuarios: any[]; locations: any[]; maxUsers: number; canCreateOwner: boolean }) {
  ...
  const handleCrear = async (e: React.FormEvent) => {
    ...
    setIsModalOpen(false); setForm({ nombre: '', email: '', password: '', rol: 'VENDEDOR', locationId: '', commissionPct: '0' }); window.location.reload();
  };
  ...
  return (
    <div className="p-6 md:p-8 max-w-7xl mx-auto space-y-6">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4"><div><p className="text-xs font-black uppercase tracking-[0.18em] text-blue-600">Gestión</p><h1 className="text-3xl font-black text-slate-900 mt-1 flex items-center gap-3"><Users className="w-8 h-8 text-blue-600" />Usuarios y accesos</h1><p className="text-sm text-slate-500 mt-1">Roles, sucursal operativa y comisión por colaborador.</p></div><button onClick={() => setIsModalOpen(true)} disabled={usuarios.length >= maxUsers} className="bg-blue-600 text-white px-5 py-2.5 rounded-xl font-bold flex items-center gap-2 hover:bg-blue-700 disabled:opacity-50"><UserPlus className="w-5 h-5" /> Nuevo colaborador</button></div>
