...
export default function ConsignacionesClient({ vehiculos, clientes, dolarActual }: { vehiculos: any[]; clientes: any[]; dolarActual: number }) {
  ...
  const [form, setForm] = useState({ marca: '', modelo: '', anio: String(new Date().getFullYear()), patente: '', km: '', comision_pct: '5' });
  ...
  const filtrados = useMemo(() => {
    ...
    return vehiculos.filter(v => !q || `${v.marca || ''} ${v.modelo || ''} ${v.patente || ''} ${v.cliente?.nombre_completo || ''}`.toLowerCase().includes(q));
  }, [vehiculos, search]);
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    setSaving(true);
    const res = await registrarConsignacion({ id_cliente: Number(clienteId), marca: form.marca.trim(), modelo: form.modelo.trim(), anio: Number(form.anio), patente: form.patente.trim(), km: Number(form.km || 0), precio_venta_ars: Number(precio.ars), comision_pct: Number(form.comision_pct || 0), cotizacion_dolar: dolarActual });
    setSaving(false);
  ...
  return (
    <div className="p-6 max-w-[1600px] mx-auto space-y-6">
      <div className="flex flex-col md:flex-row justify-between gap-4"><div><h1 className="text-3xl font-black text-slate-900 flex items-center gap-3"><CarFront className="w-8 h-8 text-fuchsia-600" /> Consignaciones</h1><p className="text-sm text-slate-500 mt-1">Unidades de terceros, comisión y liquidación en ambas monedas.</p></div><button onClick={() => setOpen(true)} className="px-5 py-2.5 bg-fuchsia-600 text-white rounded-xl font-black flex items-center gap-2"><Plus className="w-4 h-4" /> Nueva consignación</button></div>
      ...
      <div className="bg-white border border-slate-200 rounded-2xl p-4"><div className="relative"><Search className="absolute left-3 top-3 w-4 h-4 text-slate-400" /><input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Buscar unidad, patente o propietario..." className="w-full pl-9 pr-3 py-2.5 border border-slate-300 rounded-xl text-sm" /></div></div>

      <div className="bg-white border border-slate-200 rounded-2xl overflow-hidden"><div className="overflow-x-auto"><table className="w-full text-sm min-w-[1000px]"><thead className="bg-slate-50 text-[10px] uppercase text-slate-500 border-b"><tr><th className="text-left px-5 py-4">Unidad</th><th className="text-left px-5 py-4">Propietario</th><th className="text-right px-5 py-4">Precio publicado</th><th className="text-right px-5 py-4">A liquidar</th><th className="text-right px-5 py-4">Comisión agencia</th><th className="text-right px-5 py-4">Acción</th></tr></thead><tbody className="divide-y">{filtrados.map(v => { const agencyUsd = Number(v.precio_venta_usd || 0) - Number(v.precio_compra_usd || 0); const agencyArsRow = Number(v.precio_venta_ars || 0) - Number(v.precio_compra_ars || 0); return <tr key={v.id_vehiculo} className="hover:bg-slate-50"><td className="px-5 py-4"><p className="font-black text-slate-900">{v.marca} {v.modelo}</p><p className="text-xs text-slate-500">{v.anio || 'S/A'} · {v.patente || 'S/P'} · {v.estado.replace(/_/g, ' ')}</p></td><td className="px-5 py-4"><p className="font-bold text-slate-800 flex items-center gap-2"><UserRound className="w-4 h-4 text-slate-400" />{v.cliente?.nombre_completo || 'Sin titular vinculado'}</p></td><td className="px-5 py-4 text-right"><DualMoney ars={v.precio_venta_ars} usd={v.precio_venta_usd} rate={dolarActual} compact primaryClassName="font-black text-fuchsia-800" /></td><td className="px-5 py-4 text-right"><DualMoney ars={v.precio_compra_ars} usd={v.precio_compra_usd} rate={dolarActual} compact /></td><td className="px-5 py-4 text-right"><DualMoney ars={agencyArsRow} usd={agencyUsd} rate={dolarActual} compact primaryClassName="font-black text-emerald-700" /><p className="text-[10px] text-slate-400">{Number(v.comision_consignacion_pct || 0)}%</p></td><td className="px-5 py-4 text-right"><button disabled={v.estado !== 'VENDIDO'} onClick={() => liquidar(v)} className="px-3 py-2 rounded-lg bg-slate-900 text-white text-xs font-black disabled:opacity-30">Liquidar</button></td></tr>; })}{filtrados.length === 0 && <tr><td colSpan={6} className="p-12 text-center text-slate-500">No hay consignaciones para este filtro.</td></tr>}</tbody></table></div></div>

      {open && <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4"><form onSubmit={submit} className="bg-white w-full max-w-3xl rounded-3xl p-6 shadow-2xl space-y-5"><div className="flex justify-between"><div><h3 className="text-xl font-black text-slate-900">Nueva consignación</h3><p className="text-xs text-slate-500 mt-1">Seleccioná propietario y definí condiciones comerciales.</p></div><button type="button" onClick={() => setOpen(false)}><X className="w-5 h-5 text-slate-400" /></button></div><SearchCombobox label="Propietario / consignante" required value={clienteId} onChange={setClienteId} options={clientOptions} placeholder="Buscar por nombre, DNI, CUIT o teléfono..." /><div className="grid grid-cols-1 md:grid-cols-2 gap-4"><label><span className="text-xs font-black uppercase text-slate-600">Marca *</span><input required value={form.marca} onChange={(e) => setForm({ ...form, marca: e.target.value })} className="mt-1.5 w-full border border-slate-300 rounded-xl px-3 py-2.5" /></label><label><span className="text-xs font-black uppercase text-slate-600">Modelo *</span><input required value={form.modelo} onChange={(e) => setForm({ ...form, modelo: e.target.value })} className="mt-1.5 w-full border border-slate-300 rounded-xl px-3 py-2.5" /></label><label><span className="text-xs font-black uppercase text-slate-600">Año *</span><input type="number" required value={form.anio} onChange={(e) => setForm({ ...form, anio: e.target.value })} className="mt-1.5 w-full border border-slate-300 rounded-xl px-3 py-2.5" /></label><label><span className="text-xs font-black uppercase text-slate-600">Patente</span><input value={form.patente} onChange={(e) => setForm({ ...form, patente: e.target.value.toUpperCase() })} className="mt-1.5 w-full border border-slate-300 rounded-xl px-3 py-2.5 uppercase" /></label><label><span className="text-xs font-black uppercase text-slate-600">Kilometraje</span><input type="number" min="0" value={form.km} onChange={(e) => setForm({ ...form, km: e.target.value })} className="mt-1.5 w-full border border-slate-300 rounded-xl px-3 py-2.5" /></label><label><span className="text-xs font-black uppercase text-slate-600">Comisión agencia (%) *</span><input type="number" min="0" max="100" step="0.01" value={form.comision_pct} onChange={(e) => setForm({ ...form, comision_pct: e.target.value })} className="mt-1.5 w-full border border-slate-300 rounded-xl px-3 py-2.5" /></label></div><DualCurrencyInput label="Precio de publicación" required ars={precio.ars} usd={precio.usd} rate={dolarActual} onChange={setPrecio} /><div className="grid grid-cols-1 md:grid-cols-2 gap-3"><div className="bg-amber-50 border border-amber-100 rounded-xl p-3"><p className="text-[10px] uppercase font-black text-amber-700">Monto estimado del propietario</p><DualMoney ars={ownerArs} rate={dolarActual} /></div><div className="bg-emerald-50 border border-emerald-100 rounded-xl p-3"><p className="text-[10px] uppercase font-black text-emerald-700">Comisión estimada agencia</p><DualMoney ars={agencyArs} rate={dolarActual} /></div></div><div className="flex justify-end gap-3 border-t pt-4"><button type="button" onClick={() => setOpen(false)} className="px-4 py-2.5 bg-slate-100 rounded-xl font-bold">Cancelar</button><button disabled={saving} type="submit" className="px-5 py-2.5 bg-fuchsia-600 text-white rounded-xl font-black flex items-center gap-2">{saving && <Loader2 className="w-4 h-4 animate-spin" />} Registrar</button></div></form></div>}
    </div>
  );
}
